...
class ChattingService {
    ...
    async expGain(user) {
        if (
            Utils.timeSinceEpoch() - user.chatting.lastMessageTime <
            CHATTING_EXP_DELAY_IN_SECONDS
        )
            return false;

        const gainedExperience =
            Utils.randomInt(CHATTING_EXP_MIN_GAIN, CHATTING_EXP_MAX_GAIN) *
            CHATTING_EXP_MULTIPLIER;

        const hasLeveledUp = this.hasLeveledUp(
            user.chatting.level,
            user.chatting.expTowardsNextLevel + gainedExperience,
        );
        ...
        if (hasLeveledUp[0]) {
            ...
            updatedFields.chatting.expTowardsNextLevel =
                user.chatting.expTowardsNextLevel + gainedExperience;
        }
        ...
        if (hasLeveledUp[0]) return true;
        else return false;
    }

    hasLeveledUp(level, expTowardsNextLevel) {
        const expNeededToLevelUp =
            ChattingService.calculateExpNeededToLevelUp(level);

        if (expTowardsNextLevel >= expNeededToLevelUp) {
            const newExpTowardsNextLevel =
                expTowardsNextLevel - expNeededToLevelUp;
