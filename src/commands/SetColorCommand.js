...
class SetColorCommand extends Command {
            ...
            new SlashCommandBuilder()
                ...
                .addStringOption((option) =>
                    option
                        .setName("hexcode")
                        .setDescription(
                            `Color code (Hexadecimal, e.g.: "${defaultColor}; use a color picker to find the hex code of a color)`,
                        )
                        .setRequired(true),
                ),
            context,
    ...
    async execute(interaction, client) {
        ...
        if (!hexCodeInput.startsWith("#")) hexCodeInput = `#${hexCodeInput}`;
        ...
        switch (SetColorCommand.#VALIDHEX.test(hexCodeInput)) {
            case true:
                ...
                const { embed, attachment } =
                    await this.context.profileService.generateSetColor(
                        hexCodeInput,
                    );
                interaction.reply({
